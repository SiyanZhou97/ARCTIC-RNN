...
def create_cosine_bumps_torch(x, centers, widths,device):
    """Create cosine bumps.
    ...
    widths=torch.FloatTensor(widths).to(device)
    ...
    bases = torch.empty((x.shape[0], centers.shape[0])).to(device)
    # Loop over center positions
    for idx, cent in enumerate(centers):
        bases[:, idx] = (torch.cos(2 * np.pi * (x_reshape - cent) / widths[idx]) * 0.5 + 0.5) * \
                    (torch.absolute(x_reshape - cent) < widths[idx] / 2)
    return bases

def position_expansion_torch(pos,n_pos_bases=5,start_pos=0,end_pos=1,device='cpu'):
    ...
    pos_bases = create_cosine_bumps_torch(pos, pos_centers, pos_width * np.ones_like(pos_centers),device) # position basis expansion
    return pos_bases